...
const Section_9009b281 = ((
  ...
  function TestimonialRow({ reviews }: { reviews: ReviewData[] }) {
    ...
    // The anchor the row was resting on right before the current swipe/scroll
    // gesture began. Captured once per gesture (see handleScroll) so the
    // gesture's landing spot can be clamped to at most one card/gap away from
    // it, however far or fast the person actually swiped.
    const gestureStartAnchorRef = useRef<number | undefined>(undefined);
    ...
    const handleScroll = () => {
      ...
      // The first scroll event of a new gesture (no pending scroll-end timer
      // yet) is when we snapshot where the row was resting — that's the
      // anchor every subsequent event in this same gesture gets clamped
      // against, regardless of how far the finger/momentum actually carries
      // the scroll position.
      if (scrollEndTimeoutRef.current === undefined) {
        gestureStartAnchorRef.current = anchorRef.current;
      }
      ...
      scrollEndTimeoutRef.current = window.setTimeout(() => {
        ...
        // Whichever card/gap is nearest to center right now — but no matter
        // how hard or fast the swipe was, the actual landing spot is capped
        // to one step (one card in 'card' mode, one gap in 'gap' mode) away
        // from wherever the gesture started. A single vigorous swipe can
        // otherwise carry the scroll position past several cards at once;
        // this makes every swipe move exactly one review, same as an arrow
        // click, while still following the direction/rough distance of the
        // raw nearest-anchor result.
        const rawNearest = nearestAnchor(mode);
        const startAnchor = gestureStartAnchorRef.current ?? naturalAnchor(total, mode);
        const step = mode === 'card' ? 1 : 0.5;
        let next = startAnchor;
        if (rawNearest > startAnchor) next = startAnchor + step;
        else if (rawNearest < startAnchor) next = startAnchor - step;
        ...
        gestureStartAnchorRef.current = undefined;
