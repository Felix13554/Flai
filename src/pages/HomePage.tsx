...
const Section_9009b281 = (() => {
  // ─── Types ────────────────────────────────────────────────────────────────────
  //
  // Each row is ONE content key holding a JSON ARRAY of review objects (not one key
  // per review). The array items mirror the shape coming from cached_reviews.reviews_data
  // so you can copy-paste straight from the Supabase table without reformatting.
  //
  // Required fields per item:  user.name, rating, snippet
  // Optional fields per item:  user.thumbnail, link, business
  //
  // Content keys:
  //   testimonials-review-1   -> array of reviews for row 1
  //   testimonials-review-2   -> array of reviews for row 2
  //
  // Example value for testimonials-review-1:
  // [
  //   {
  ...
  // of every review across both rows.
  //
  // ─── Layout rules ────────────────────────────────────────────────────────────
  // - 8 or fewer reviews total (row 1 + row 2 combined) -> everything renders as a single row
  // - more than 8 reviews total                          -> row 1 and row 2 render as configured
  // - a row with fewer than 4 reviews  -> that row is static, no scroll animation
  // - a row with 4 or more reviews     -> that row scrolls continuously (row 1 left, row 2 right)
  ...
  const ROW_1_KEY = 'testimonials-review-1';
  const ROW_2_KEY = 'testimonials-review-2';
  ...
  /** Parses a content key holding a JSON array of review objects. */
  ...
  function TestimonialRow({
    ...
    // Duplicate the set so the loop is seamless (translate exactly 50% of the doubled track).
    const cards = animated ? [...reviews, ...reviews] : reviews;
    const trackClass = animated
      ? direction === 'left'
        ? 'testi-row-track testi-scroll-left'
        : 'testi-row-track testi-scroll-right'
      : 'testi-row-track testi-row-static';

    return (
      <div className="testi-row-viewport">
        <div
          className={trackClass}
          style={animated ? { animationDuration: `${reviews.length * 7}s` } : undefined}
        >
          {cards.map((review, i) => (
  ...
  const Testimonials: React.FC = () => {
    ...
    const configuredRow1 = parseReviewArray(getContent(ROW_1_KEY, ''));
    const configuredRow2 = parseReviewArray(getContent(ROW_2_KEY, ''));
    const allReviews = [...configuredRow1, ...configuredRow2];
    ...
    // 8 or fewer total -> merge into a single row, ignoring the row 1/row 2 split.
    // More than 8 total -> keep row 1 and row 2 as configured.
    const showSecondRow = total > 8 && configuredRow2.length > 0;
    const row1 = showSecondRow ? configuredRow1 : allReviews;
    const row2 = showSecondRow ? configuredRow2 : [];
    ...
    return (
      <section className="py-20">
        <style>{`
          ...
          .testi-scroll-left {
            animation-name: testiScrollLeft;
            animation-timing-function: linear;
            animation-iteration-count: infinite;
          }
          .testi-scroll-right {
            animation-name: testiScrollRight;
            animation-timing-function: linear;
            animation-iteration-count: infinite;
          }
          .testi-row-viewport:hover .testi-scroll-left,
          .testi-row-viewport:hover .testi-scroll-right {
        ...
        <div className="testi-header">
          ...
          >
            ...
            Bedømt {Number.isInteger(ratingData.rating) ? ratingData.rating : ratingData.rating.toFixed(1)}/5 af over {ratingData.reviewCount} kunder på Google
          </a>
          <h2>Ord fra andre om vores tilstedeværelse</h2>
        </div>

        <div className="testi-rows">
          <TestimonialRow reviews={row1} animated={row1.length >= 4} direction="left" />
          {showSecondRow && (
            <TestimonialRow reviews={row2} animated={row2.length >= 4} direction="right" />
          )}
        </div>
      </section>
    );
...
export const DEPLOYED_HOME_SECTIONS = [
  ...
  },
  {
    ...
    "title": "Mød Felix",
    "description": "13-årige Felix er ejeren af Flai og vores dronepilot med en passion for dronefotografering og -optagelser og 4 års erfaring.",
