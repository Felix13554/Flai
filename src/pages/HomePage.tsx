...
const Section_9009b281 = ((
  ...
  // finished work underneath that review's card:
  ...
  //   - omitted / empty string -> nothing is rendered, card stays text-only
  //
  ...
  // Each card's "result" (see field docs above) renders below the author row,
  // inside the same card. Because it lives inside the card, it scrolls with it
  // and is covered by the row viewport's existing left/right edge fade
  // (.testi-row-viewport's mask-image) automatically — no separate fade logic
  // needed for it.
  ...
  function ReviewCard({ review }: { review: ReviewData }) {
    ...
    // "result" is optional: empty/omitted -> render nothing. A YouTube URL
    // renders as an embedded video; any other URL is treated as an image.
    ...
    return (
      <div className="testi-review-card">
        ...
        {youtubeEmbedUrl ? (
          <div className="testi-result-media">
            <iframe
              src={youtubeEmbedUrl}
    ...
    return (
      <section className="py-20">
        <style>{`
          ...
          .testi-result-media iframe,
          .testi-result-media img {
            display: block;
            width: 100%;
            height: 100%;
            border: none;
