...
const Section_9009b281 = ((
  ...
  function TestimonialRow({ reviews }: { reviews: ReviewData[] }) {
    ...
    // intended to change cards, rather than an incidental finger wobble.
    ...
    const handleTouchStart = (e: React.TouchEvent) => {
      // Don't interrupt an animation already in flight — let it finish so a
      // second swipe can't stack a partial jump on top of one still playing.
    ...
    const handleTouchMove = (e: React.TouchEvent) => {
      if (touchStartXRef.current === null || touchDecidedRef.current) return;
      const dx = e.touches[0].clientX - touchStartXRef.current;
      if (Math.abs(dx) < SWIPE_THRESHOLD_PX) return;

      // Threshold crossed: this gesture is now a swipe. A swipe is a
      // shortcut for the arrow button, not a separate way of moving the
      // row — so instead of re-running the "advance one card" logic
      // ourselves, we literally click the corresponding arrow button.
      // That means a swipe automatically gets everything the button
      // already has for free: the exact same handler, the same disabled/
      // clamped-at-the-end behavior (a native .click() on a disabled
      // button is a no-op, so swiping past the last review does nothing,
      // same as clicking a greyed-out arrow), and it can never drift out
      // of sync with what the button does, because it IS the button.
      touchDecidedRef.current = true;
      e.preventDefault();
      const targetButton = dx < 0 ? rightArrowRef.current : leftArrowRef.current;
      targetButton?.click();
    };
    ...
    useEffect(() => {
      ...
      // (a debounced settle-on-scroll would fire, then get its result
      // immediately discarded by the next reset).
      ...
      <section className="py-20">
        <style>{`
          ...
          .testi-row-viewport {
            ...
            flex: 1;
            min-width: 0;
            /* Blocks the browser's own native horizontal pan/momentum scroll
               on touch devices (vertical page scroll still passes through).
               Swiping is handled entirely by our touch handlers + the same
               scrollToAnchor animation the arrows use, so native horizontal
               scrolling here would otherwise fight it and look glitchy. */
            touch-action: pan-y;
