...
const Section_9009b281 = ((
  ...
  function TestimonialRow({ reviews }: { reviews: ReviewData[] }) {
    ...
    const handleTouchMove = (e: React.TouchEvent) => {
      ...
      // Threshold crossed: this gesture is now a swipe. Immediately fire the
      // exact same one-step animation as clicking an arrow — same duration,
      // same easing, same distance — and stop here, so nothing about how
      // hard, fast, or far the finger keeps moving can change the outcome.
      // preventDefault stops the browser's own native horizontal scroll/
      // momentum from also kicking in and fighting our animation over
      // scrollLeft (that fight was the "speeds up / glitchy" feel).
      touchDecidedRef.current = true;
      e.preventDefault();
      scrollByOneCard(dx < 0 ? 'right' : 'left');
    };
