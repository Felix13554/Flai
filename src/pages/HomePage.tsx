...
const Section_9009b281 = ((
  ...
  function TestimonialRow({ reviews }: { reviews: ReviewData[] }) {
    ...
    /** Scrolls (no animation — this only runs on initial layout) so the card
     * at `index` sits centered in the viewport, clamped so it never
     * overshoots the scrollable range at either end. */
    const centerCardAtIndex = (index: number) => {
      ...
      const cardCenterFromTrackLeft = (cardRect.left - trackRect.left) + cardRect.width / 2;
      const targetLeft = viewport.scrollLeft + cardCenterFromTrackLeft - viewport.clientWidth / 2;
      const maxScroll = viewport.scrollWidth - viewport.clientWidth;

      viewport.scrollLeft = Math.max(0, Math.min(targetLeft, maxScroll));
    };
    ...
    useEffect(() => {
      ...
      // Center the middle review (e.g. review 2 of 4) once layout has
      // settled — measuring synchronously on mount was the source of the
      // old "jumps to a random card / shows blank" bug, so this waits a
      // frame for card widths and side padding to be final first.
