...
const Section_9009b281 = ((
  ...
  //
  // The row loops infinitely: a clone of the last review sits before the first
  // (so clicking "back" from review 1 lands on what looks like the last review),
  // and a clone of the first review sits after the last, for the same effect in
  // reverse. Once the scroll settles on a clone, we silently snap (no animation)
  // to the matching real card at the identical scroll position — invisible to
  // the user.
  ...
  function TestimonialRow({ reviews }: { reviews: ReviewData[] }) {
    ...
    // Looping only makes sense with more than one review. When looping, we
    // prepend a clone of the last review and append a clone of the first, so
    // arrow clicks and swipes can move seamlessly past either end.
    const loopEnabled = reviews.length > 1;
    const displayReviews = loopEnabled
      ? [reviews[reviews.length - 1], ...reviews, reviews[0]]
      : reviews;
    const firstRealIndex = loopEnabled ? 1 : 0;

    const getCardStep = (): number | null => {
    ...
    const updateSidePadding = () => {
      const track = trackRef.current;
      if (!track) return;
      // Fixed, small inset rather than centering the visible block as a
      // group — the first review should sit at the left edge of the row,
      // not mid-screen with matching blank space on both sides.
      track.style.setProperty('--testi-side-pad', `${CARD_GAP_PX}px`);
    };

    const visibleCardCount = (): number => {
    ...
    const updateArrowState = () => {
      ...
      // gain from wrapping either — disable both arrows.
      ...
      if (loopEnabled) {
        // Infinite scroll: there's always somewhere to go in both directions.
    ...
    /** If the viewport has settled on a cloned card at either end, instantly
     * (no animation) snap to the matching real card at the identical scroll
     * position — the jump is invisible since the clone and the real card
     * render identically. */
    const resolveLoopWrap = () => {
      ...
      if (viewport.scrollLeft < threshold) {
        // Sitting on the leading clone (a copy of the last review) — jump
        // forward by the full real-review span to land on the actual last one.
        viewport.scrollLeft += reviews.length * step;
      } else if (viewport.scrollLeft > viewport.scrollWidth - viewport.clientWidth - threshold) {
        // Sitting on the trailing clone (a copy of the first review) — jump back.
        viewport.scrollLeft -= reviews.length * step;
      }
    };
    ...
    const handleScroll = () => {
      ...
      // Fallback for native swipe on browsers without the 'scrollend' event:
      // resolve shortly after scrolling goes idle. Kept short since it's a
      // fallback, not the primary mechanism (see the scrollend listener below).
    ...
    useEffect(() => {
      ...
      // Start on the first real review (skipping the leading clone in looped
      // rows), in the same order as the JSON — no auto-centering on mount.
      const step = getCardStep();
      viewport.scrollLeft = loopEnabled && step ? firstRealIndex * step : 0;
      updateSidePadding();
      updateArrowState();
      ...
      // Primary loop-wrap trigger: fires the instant the browser considers the
      // scroll (including native swipe/snap) settled — no guessed delay.
      const onScrollEnd = () => resolveLoopWrap();
    ...
    const scrollByOneCard = (dir: 'left' | 'right') => {
      ...
      const step = getCardStep();
      if (!viewport || !step) return;
      const targetLeft = viewport.scrollLeft + (dir === 'left' ? -step : step);
      // Resolve the wrap the instant the animation itself finishes — exact,
      // not a guessed timeout.
      animateScrollTo(viewport, targetLeft, SCROLL_ANIMATION_MS, () => resolveLoopWrap());
    };
  ...
  const Testimonials: React.FC = () => {
    ...
    const rawReviews = getContent(REVIEWS_KEY, '');
    const rawRating = getContent(RATING_KEY, '');

    // Memoized on the raw content string, not recomputed as a fresh array on
    // every render — otherwise an unrelated re-render (e.g. from elsewhere in
    // the app) hands TestimonialRow a "new" reviews array by reference even
    // though nothing actually changed, which resets its scroll position.
    const allReviews = useMemo(() => parseReviewArray(rawReviews), [rawReviews]);
    const total = allReviews.length;

    const ratingData = useMemo(
      () => parseRating(rawRating) ?? fallbackRating(allReviews),
      [rawRating, allReviews]
    );
