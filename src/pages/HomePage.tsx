...
const Section_9009b281 = ((
  ...
  //
  // ─── Thumbnail shape ─────────────────────────────────────────────────────────
  // Real Google profile photos are opaque photographs — cropping them into a
  // circle looks natural. Some "reviewers" are actually businesses whose
  // thumbnail is a logo with a transparent background; cropping a logo into a
  // circle clips it. Each thumbnail is probed once (via a hidden canvas) for
  // any non-opaque pixel. If it has transparency it's treated as a logo and
  // rendered as a rounded square showing the full image instead of a cropped
  // circle. Photos (fully opaque) keep the circular avatar as before.
  ...
  interface ReviewData {
    ...
    business?: string;
    /**
     * Not in cached_reviews — add manually to override the automatic shape
     * detection. Automatic detection only works when the thumbnail file
     * actually has an alpha channel (PNG/WebP with transparency); a logo
     * saved as a plain JPEG has no transparency to detect and will default
     * to circular. Set `"logo": true` to force the square/contain treatment
     * for those, or `"logo": false` to force a circular crop even if
     * transparency happens to be detected.
     */
    logo?: boolean;
  }
  ...
  // In-memory cache so the same thumbnail URL is only probed once, even if it
  // shows up in multiple ReviewCard mounts (e.g. after a re-render or if the
  // same reviewer appears twice).
  const transparencyCache = new Map<string, boolean>();

  // Cache of processed (background-stripped) data URLs, keyed by source URL.
  const logoProcessCache = new Map<string, string>();

  /**
   * Loads an image into a hidden canvas and checks whether any pixel has an
   * alpha value below full opacity. Resolves `true` (treat as logo) only when
   * transparency is actually detected; any failure — load error, or a tainted
   * canvas from a host that doesn't allow anonymous CORS reads — safely
   * resolves `false` so the avatar just falls back to the normal circular photo
   * treatment instead of breaking.
   */
  function checkThumbnailTransparency(url: string): Promise<boolean> {
    const cached = transparencyCache.get(url);
    ...
    return new Promise((resolve) => {
      ...
      const finish = (result: boolean) => {
        transparencyCache.set(url, result);
        resolve(result);
      };

      img.onload = () => {
        try {
          // Downscaled sample canvas — we only need to know whether any
          // transparency exists, not preserve fine detail.
          const SAMPLE = 32;
          ...
          for (let i = 3; i < data.length; i += 4) {
            // Small tolerance for JPEG-style near-255 rounding noise; real
            // transparency reads meaningfully below full opacity.
            if (data[i] < 250) {
              return finish(true);
            }
          }
          ...
          // Tainted canvas (no CORS headers) or other read failure.
          finish(false);
        }
      };
  ...
  /**
   * Many "logo" thumbnails don't have a real alpha channel at all — they're a
   * flat image with a solid white (or near-white) background painted directly
   * into the pixels. CSS can't remove that; it has to be edited out of the
   * bitmap. This loads the image at full resolution, and fades any near-white
   * pixel down to transparent (with a soft-edged threshold so anti-aliased
   * edges around logo artwork don't get a hard cutout line), producing a PNG
   * data URL that drops cleanly onto the dark card. Falls back to the original
   * URL if the canvas can't be read (no CORS) or the image fails to load.
   */
  function stripNearWhiteBackground(url: string): Promise<string> {
    const cached = logoProcessCache.get(url);
    ...
    return new Promise((resolve) => {
      ...
      img.onload = () => {
        try {
          ...
          if (!ctx || canvas.width === 0 || canvas.height === 0) return resolve(url);

          ctx.drawImage(img, 0, 0);
          const imageData = ctx.getImageData(0, 0, canvas.width, canvas.height);
          const data = imageData.data;

          // min(r,g,b) at/above this -> fully transparent.
          const FULLY_TRANSPARENT_AT = 246;
          // min(r,g,b) below this -> untouched (keep original alpha).
          const FADE_STARTS_AT = 200;
          ...
          for (let i = 0; i < data.length; i += 4) {
            ...
            if (minChannel >= FULLY_TRANSPARENT_AT) {
              data[i + 3] = 0;
            } else if (minChannel > FADE_STARTS_AT) {
              const fade = (minChannel - FADE_STARTS_AT) / (FULLY_TRANSPARENT_AT - FADE_STARTS_AT);
          ...
          logoProcessCache.set(url, dataUrl);
          resolve(dataUrl);
        } catch {
          // Tainted canvas (no CORS headers) or other failure — fall back to
          // the original image rather than breaking the thumbnail.
          resolve(url);
        }
      };
  ...
  function ReviewCard({ review }: { review: ReviewData }) {
    ...
    const [displaySrc, setDisplaySrc] = useState<string | undefined>(review.user.thumbnail);
    const [thumbFailed, setThumbFailed] = useState(false);

    useEffect(() => {
      let cancelled = false;
      setThumbFailed(false);
      setDisplaySrc(review.user.thumbnail);

      const resolve = async () => {
        // A manual "logo" flag always wins — it's the only way to force the
        // square treatment for logos saved without an alpha channel (plain
        // JPEGs), which have nothing for the pixel probe to detect.
        let logo: boolean;
        if (typeof review.logo === 'boolean') {
          logo = review.logo;
        } else if (review.user.thumbnail) {
          logo = await checkThumbnailTransparency(review.user.thumbnail);
        } else {
          logo = false;
        }
        if (cancelled) return;
        ...
        // Once something is confirmed a logo, also strip any baked-in
        // near-white background so it doesn't show as a solid box.
        if (logo && review.user.thumbnail) {
          const processed = await stripNearWhiteBackground(review.user.thumbnail);
          if (!cancelled) setDisplaySrc(processed);
        }
      };
    ...
    }, [review.user.thumbnail, review.logo]);

    const showThumbnail = !!displaySrc && !thumbFailed;
          ...
          >
            {showThumbnail ? (
              <img
                ...
                className={`author-avatar${isLogo ? ' author-avatar-logo' : ''}`}
  ...
  function TestimonialRow({ reviews }: { reviews: ReviewData[] }) {
    ...
    useEffect(() => {
      ...
      // Always start at the first review, in the same order as the JSON —
      // no auto-centering on mount (that was jumping to a random middle
      // card, and could show blank if it ran before layout settled).
      viewport.scrollLeft = 0;
      updateSidePadding();
      updateArrowState();

      // Re-check arrow availability and side padding if the row's size changes
      // (e.g. resize across the mobile breakpoint, or fonts/images loading in late).
    ...
    return (
      <div className="testi-row-wrap">
        ...
        <div className="testi-row-viewport" ref={viewportRef} onScroll={updateArrowState}>
    ...
    return (
      <section className="py-20">
        <style>{`
          ...
          /* Logo thumbnails (any transparent pixel detected, or the manual
             "logo": true override) skip the circular crop entirely — shown as
             a rounded square with the full image visible (object-fit: contain)
             instead of cropped into a circle. No added background box: the
             logo's own transparent/white pixels sit directly on the card. */
          .author-avatar-logo {
            border-radius: 10px;
            object-fit: contain;
          }
  ...
  React.useState, React.useEffect, React.useRef, React.createElement
) as React.ComponentType;
