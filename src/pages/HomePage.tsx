  ...
  function ReviewCard({ review }: { review: ReviewData }) {
    ...
    useEffect(() => {
      ...
      if (review.user.thumbnail) {
        checkThumbnailTransparency(review.user.thumbnail).then((hasTransparency) => {
          if (!cancelled) setIsLogo(hasTransparency);
        });
      } else {
        setIsLogo(false);
      }
    ...
    const showThumbnail = !!review.user.thumbnail && !thumbFailed;
    ...
    return (
      <section className="py-20">
        <style>{`
          ...
          /* Logo thumbnails (any transparent pixel detected) skip the circular
             crop entirely — shown as a rounded square with the full image
             visible (object-fit: contain) on a light backing so logos with
             dark artwork or transparent backgrounds stay legible on the dark
             card. */
          .author-avatar-logo {
            border-radius: 10px;
            object-fit: contain;
