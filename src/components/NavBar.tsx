...
const NavBar: React.FC = () => {
  ...
  // The Flai wordmark itself — same 'logo' kind ClientLogosBar/HeroProjectCarousel
  // use for image logos, sampling the hero video luminance directly behind the
  // top-left mark. Only the top-left <img> (always visible, atHero-gated below)
  // gets this — the duplicate logo inside the mobile slide-out panel sits on a
  // solid bg-neutral-900 background and never overlaps the hero video, so it
  // keeps its plain, un-shadowed style.
  const [siteLogoShadowRef, siteLogoShadowStyle] = useAdaptiveShadow<HTMLImageElement>('logo');
  ...
  // The logo's compact top-left position/style (previously only reached
  // after scrolling) is now its permanent position — always shown, still
  // with no background of its own (just the transparent <img>), regardless
  // of scroll position or route.
  const showLogoInNav = true;
  ...
  const atHero = !(isScrolled || location.pathname !== '/');
    ...
    <nav className={navbarClasses}>
      <div className="container flex justify-between items-center">
        <Link 
          ...
          tabIndex={showLogoInNav ? undefined : -1}
        >
          <img
            ref={siteLogoShadowRef}
            style={atHero ? siteLogoShadowStyle : undefined}
            src={getContent('site-logo', "/Logo.webp", 'image')}
            alt="Flai.dk"
            width="120"
            height="40"
            ...
            <div className="relative group">
              ...
              <div className="absolute right-0 mt-3 w-64 bg-neutral-800 rounded-lg shadow-xl overflow-hidden opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300">
                ...
                <div className="py-1">
                  ...
                  {isAdmin && (
                    <Link to="/finance" className="flex items-center px-4 py-2.5 text-neutral-200 hover:bg-neutral-700 transition-colors">
      ...
      <div id="mobile-menu" className={mobileMenuClasses}>
        <div className="p-5 flex-shrink-0">
          <div className="flex justify-between items-center mb-6">
            ...
            <button onClick={toggleMenu} aria-label="Luk menu">
